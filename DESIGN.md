...
- Mobile shell (app-like below `lg`): fixed bottom tab bar (`MobileTabBar`) for primary destinations; **More** opens a bottom sheet (`MobileNav`) with scrollable `SidebarNav` + pinned identity footer. Sticky top title bar with borderless 44px icon actions. Classification strip and desktop case tabs hidden on small screens. `viewport-fit=cover` + safe-area padding on topbar/tabbar/main. `PageHeader` may use `hideTitleOnMobile` when the top bar already shows the title. Touch targets ≥44px. Dense tables switch to stacked rows below `md`.
